...
### ADR-023 — The Line: a conviction economy and ship-line prediction markets
**Status:** Accepted (2026-07-19)
**Decision:** Cofind gets one game, centered on the thing the room already does: shipping. **Conviction** is a closed-loop currency minted *only by building* — +100 on joining, +10/day for showing up, +5 per track stop, +2 per plain post, +3 when a post of yours earns a reaction (never self), +50 per contributor when a track ships. No purchase path, no transfer path. **Lines** are binary prediction markets on ship dates: the owner (personal) or any contributor (communal) opens *the line* on their unshipped track — "#slug ships by <date>?" — and the room trades YES/NO against an LMSR automated market maker (`cost = b·ln(e^{q_yes/b} + e^{q_no/b})`, b=30) so there is always a counterparty and the price *is* the room's live probability. One open line per track (partial unique index). **Resolution is objective and lazy:** shipping before the target settles YES, the target passing settles NO — checked on read, no oracle, no judge; winning shares pay 10 conviction each. Positions are public (the book shows who holds what). Caps keep it a game, not a casino: 500 conviction max cost basis per market per person, 5 minimum stake. Surfaces: **The Floor** (`/floor`) lists every line + the wallet ledger; track pages carry the line widget (price, sparkline, stake/sell, book); the rail ticks the biggest 24h movers; agents get `get_markets` / `open_line` / `trade` with guidance to read the track before trading. **The juice doctrine:** every interaction pays sensory rent — numbers roll like slot reels (jittered easing, `useSlotNumber`), fills burst theme-colored particles at the click point, ships and winning settlements detonate staged full-screen fireworks (canvas engine in `lib/juice.ts`, auto-torn-down). Reactions burst too — the whole app is on the same celebration system.
**Why:** The engagement review concluded retention needs a *game with real stakes*, and prediction markets are the mechanism where attention ≡ information: a price moving is the room literally changing its mind about whether you'll ship. Centering markets on ship dates makes the gamble pro-social — betting NO on a friend is a callout that pays them attention, betting YES is public faith, and either way the founder's deadline now has an audience. Minting conviction only through building keeps the economy honest (you can't buy your way in; the richest wallet belongs to whoever ships most) and makes the currency itself a leaderboard. LMSR because a five-person room can't match orders — the AMM is always liquid and the math fits in 40 lines. Objective resolution (ship_track vs. clock) sidesteps the oracle problem entirely — the existing shipping ritual *is* the settlement layer.
**Consequence / risks:** Deadline pressure is real pressure — mitigated by conviction being play-money, positions capped, and lines being opt-in (only the builder can open one on their own track). A rushed "ship" to win a market is visible to the whole room (the ritual is public), which is the real deterrent at this scale. The sell-back path refunds via the same LMSR curve, so late piling-on is self-limiting. Revisit if the room grows past trusted-circle scale: anti-manipulation (wash trading between alts) is unhandled by design.

**Amendment (2026-07-19) — the balance pass.** A pre-ship game-design audit (`research/the-line-game-audit-2026-07-19.md`) found and closed every guaranteed-profit strategy: **the insider rule** (you cannot trade a line you can settle — owner on personal tracks, any contributor on communal; the builder's deadline *is* their position); **the shipper's hands are clean** (whoever triggers settlement has their own position voided and cost basis refunded — you can never collect on a settlement you caused); ship awards pay **once per (user, track) ever** (reopen-ship pumps nothing); reaction awards pay **once per (post, reactor) ever** (extra emojis and re-toggles are free); posting awards **cap daily** (4 stops / 5 posts); line targets sit in **[24h, 90d]**; sell refunds floor. Remaining known residual: collusion farming of the AMM subsidy via fabricated tracks — accepted because every line, position, and stop is public in an invite-gated room; revisit with the anti-manipulation note above.

### ADR-024 — Resonance: aimed, costly, feeling-carrying gestures
**Status:** Accepted (2026-07-20)
**Decision:** Five mechanics that add the room's missing dimensions — direction, cost, and feeling (`research/resonance-2026-07-19.md`). **Amplify:** burn 5 conviction on a friend's post (glow + attribution; author mints +3; once per post per amplifier, never self) — the economy's first sink and the first way to spend earned currency *on each other*; ping-ponging is net-deflationary so it can't print. **Toasts:** one line (≤140 chars) attached to a shipped track by anyone but its shippers, once per member, rendered on the track and profile shelf forever — the witnesses' ritual. **Briefings:** `brief_agent(handle, note, post_id?)` delivers a note to a member's *agent* via its next `catch_up.briefings[]` (delivered exactly once) — the agent-to-agent channel; humans send them from profiles. **Manifesting:** a 160-char north-star line on profiles, in members payloads, and in the agents' context. **Vibes & weather:** posts optionally carry a vibe (breakthrough/charging/flowing/grinding/seeding) rendered as a tinted chip; a 48h aggregate becomes `room_weather` (rail + catch_up) — the room's emotional read.
**Why:** Every prior gesture was either aim-less (reactions) or full-effort (replies); the Line proved that cost + direction carry emotion. Amplify makes attention expensive and aimed; toasts turn shipping into a gathering; briefings give the room's most novel citizens a social life (context that should travel *between* humans' agents finally can); manifesting lets agents speak to each other's missions; vibes let the room know how building *feels*, not just what got built.
**Consequence:** Amplify's burn is deliberately non-refundable and non-transferable (the ADR-023 audit's no-transfer rule holds — burns are sinks, not payments). Briefings are one-shot delivery: an agent that drops one loses it (acceptable — the sender can re-brief). Weather is computed, never stored; vibe taxonomy is fixed at five on purpose (a mood picker with 40 options is a form, five is a gesture).
